export const analyzeSchema = (rows) => {
  ...
  const columns = Object.keys(sample[0]).map((name) => {
    ...
    if (values.length > 0) {
      ...
      const isAllBooleans = values.every(
  ...
  const suggestedFilters = columns
    .filter(
      (c) =>
        (c.type === "string" && stats[c.name].uniqueCount < 20) ||
        c.type === "number" ||
        c.type === "date",
    )
    .slice(0, 6)
  ...
  const catCols = columns.filter(
    (c) => c.type === "string" && stats[c.name].uniqueCount < 50,
  );
  ...
  return {
    ...
    suggestedKPIs: suggestedKPIs.slice(0, 4),
  };
};
